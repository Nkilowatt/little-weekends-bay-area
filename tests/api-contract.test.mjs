...
const sourceKeys = [
  ...
  "redwood-city-events",
  "redwood-city-library-calendar",
  "redwood-city-family-calendar",
];
...
test("API reports full coverage only when every source is current", async () => {
  ...
  assert.equal(payload.currentSourceCount, 9);
  assert.equal(payload.sourceCount, 9);
...
test("API marks stale-source events for recheck without retry amplification", async () => {
  ...
  assert.equal(payload.currentSourceCount, 8);
