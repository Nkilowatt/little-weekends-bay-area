...
- Visitor photo uploads stay hidden unless the production flag, private R2 bucket, D1 migrations, and exact reviewer allowlist are all ready. The Worker uses bundled WebAssembly codecs when a platform image-transform binding is unavailable: it validates dimensions, rejects images above 20 million decoded pixels, scales to at most 1600px, and writes metadata-free WebP to R2. Uploads require rights and identifiable-person consent, remain pending until approval, support idempotent retry recovery, and expose a manual withdrawal-code fallback when browser storage is unavailable.
